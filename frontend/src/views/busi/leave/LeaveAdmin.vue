<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <p class="page-subtitle">管理全园幼儿请假记录</p>
    </div>
    ...
    <el-card shadow="never" class="content-card">
      ...
      >
        ...
        <el-table-column prop="reason" label="请假原因" min-width="120" align="center">
          <template #default="{ row }">
        ...
        <el-table-column prop="parentUserName" label="申请人" width="100" align="center">
          <template #default="{ row }">
            <span class="user-name">{{ row.parentUserName || '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="teacherName" label="审批人" width="100" align="center">
          <template #default="{ row }">
            <span class="user-name">{{ row.teacherName || '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="teacherRemark" label="审批意见" min-width="120" align="center">
          <template #default="{ row }">
            <el-tooltip :content="row.teacherRemark" placement="top" v-if="row.teacherRemark">
              <span class="remark-text">{{ row.teacherRemark }}</span>
            </el-tooltip>
            <span v-else class="no-reason">-</span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      ...
      request.get('/api/busi/leave/admin-page', { params }).then(res => {
...
.student-name, .class-name, .user-name { font-weight: 600; color: #5D4E4E; }
.cute-tag { border-radius: 20px; font-weight: 500; }
.date-text, .time-text, .days-text { color: #5D4E4E; font-size: 13px; }
.reason-text, .remark-text { display: inline-block; max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }
