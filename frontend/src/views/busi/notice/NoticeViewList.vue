<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <p class="page-subtitle">查看幼儿园通知</p>
    </div>
    ...
    <el-card shadow="never" class="content-card">
      <el-form :inline="true" :model="listQuery" class="search-form">
        ...
        <el-form-item label="确认状态" class="search-item">
          <el-select v-model="listQuery.confirmStatus" placeholder="请选择状态" clearable class="cute-select status-select">
            <el-option label="全部" :value="undefined" />
            <el-option label="✅ 已确认" :value="1" />
            <el-option label="⚠️ 待确认" :value="2" />
          </el-select>
        </el-form-item>
      ...
      >
        ...
        <el-table-column prop="targetScopeText" label="来源" width="120" align="center">
          <template #default="{ row }">
        ...
        <el-table-column prop="confirmStatus" label="状态" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="row.confirmStatus === 1 ? 'success' : 'danger'" class="cute-tag status-tag" effect="plain">
              <span v-if="row.confirmStatus === 1">✅ 已确认</span>
...
<script>
import { getViewPage } from '@/api/notice'
  ...
  methods: {
    async getList() {
      ...
      try {
        ...
        if (params.confirmStatus === '') params.confirmStatus = undefined
        const res = await getViewPage(params)
    ...
    handleDetail(row) {
      this.$router.push(`/busi/notice/view/detail/${row.id}`)
    }
  }
}
</script>
...
.type-select, .level-select, .status-select { min-width: 140px; }
